...
function myErrorHandler( $errno, $errstr, $errfile, $errline ) {
	...
	// debug_print_backtrace();
		$logs['singlewarning'] = sprintf( __('<strong>PHP Unknown error</strong> [%s] %s %s %s') , $errno , $errstr, $errfile , $errline );
		break;
	}
	return true;
}
...
$iteration = 0;
for ( $x_pos = 0, $ln = 0, $line = '', $still = true; $still && $iteration<6000 ; $x_pos-- ) {
	...
	if ( $char === "\n" ) {
		...
		if ( $deal != '' ) {
			...
			// echo $bytes." ".$data_to_parse."Satya\n";
			...
			/*
				Satya Prakash
				in order to stop showing repeated logs if there is no search, else otherwise
			*/
			if ($search==''){
				// echo "<br>before check repeat log <br>".$log['Log']."<br>";
				if ( checkRepeatLog( $log ) ) {
					// echo "<br>".getIndex($log['Log'])."\n";
					$logs['logs'][getIndex($log['Log'])]['count']=countLog($log['Log']);
					continue;
				}
				// echo "before create index <br>";
				createIndex($log['Log']);
			}
			...
			if ( is_array( $log ) ) {
				...
				foreach ( $log as $key => $value ) {
					...
					if ( ( isset( $exclude[ $key ] ) ) && ( is_array( $exclude[ $key ] ) ) ) {
						foreach ( $exclude[ $key ] as $ekey => $reg ) {
							try {
								// echo $ekey."********".$reg."********".$value."<br>";
								if ( preg_match( $reg , $value ) ) {
									
									// echo "true<br>";
									/*
										Satya Prakash
										Commented below line(wasn't supporting some format)
									*/
									// $return_log = false;
									break ;
								}
				...
				if ( $return_log === false ) {
					...
					if ( $return_log === false ) {
						...
						/*
							Satya Prakash
							do count only when there is no search, else otherwise
						*/
						
						if ($search=='')
							$log['count'] = countLog($log['Log']) ;
			...
			/*
				Satya Prakash
				change here to check for whole file
			*/
		...
		/*
			Satya Prakash
			change if dont care how much time does it take
		*/
